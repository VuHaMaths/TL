...
from .FeedForwardBase import FeedForwardBase


# Feed forward classical
# For  A, Gam
class FeedForwardGam(FeedForwardBase):
        ...
        with tf.compat.v1.variable_scope("NetWorkGam" + str(iStep), reuse=tf.compat.v1.AUTO_REUSE):
            ...
            fPrev = fc(X, int(self.layerSize[0]), scope='enc_fc1', activation_fn=self.activation,
                       weights_initializer=tf.constant_initializer(
                           np.reshape(weightInit[:self.d * self.layerSize[0]], [self.d, self.layerSize[0]])),
                       biases_initializer=tf.constant_initializer(biasInit[0:self.layerSize[0]]))
            ...
            for i in np.arange(len(self.layerSize) - 1):
                ...
                f = fc(fPrev, int(self.layerSize[i + 1]), scope=scopeName, activation_fn=self.activation,
                       weights_initializer=tf.constant_initializer(
                           np.reshape(weightInit[cMinW: cMinW + self.layerSize[i] * self.layerSize[i + 1]],
                                      [self.layerSize[i], self.layerSize[i + 1]])),
                       biases_initializer=tf.constant_initializer(biasInit[cMinB:cMinB + self.layerSize[i + 1]]))
            ...
            ZGam = fc(fPrev, sizeFin, scope='Gam', activation_fn=None, weights_initializer=tf.constant_initializer(
                np.reshape(weightInit[cMinW:cMinW + self.layerSize[len(self.layerSize) - 1] * sizeFin],
                           [self.layerSize[len(self.layerSize) - 1], sizeFin])),
                      biases_initializer=tf.constant_initializer(biasInit[cMinB:cMinB + sizeFin]))
    ...
    def createNetworkNotTrainable(self, X, iStep, weightInit, biasInit):
        with tf.compat.v1.variable_scope("NetWorkGamNotTrain" + "_" + str(iStep), reuse=False):
            ...
            fPrev = fc(X, int(self.layerSize[0]), scope='enc_fc1', activation_fn=self.activation,
                       weights_initializer=tf.constant_initializer(
                           np.reshape(weightInit[:self.d * self.layerSize[0]], [self.d, self.layerSize[0]])),
                       biases_initializer=tf.constant_initializer(biasInit[0:self.layerSize[0]]), trainable=False)
            cMinW += self.d * self.layerSize[0]
            cMinB += self.layerSize[0]
            for i in np.arange(len(self.layerSize) - 1):
                scopeName = 'enc_fc' + str(i + 2)
            ...
            ZGam = fc(fPrev, sizeFin, scope='Gam', activation_fn=None, weights_initializer=tf.constant_initializer(
                np.reshape(weightInit[cMinW:cMinW + self.layerSize[len(self.layerSize) - 1] * sizeFin],
                           [self.layerSize[len(self.layerSize) - 1], sizeFin])),
                      biases_initializer=tf.constant_initializer(biasInit[cMinB:cMinB + sizeFin]), trainable=False)
        return tf.reshape(ZGam, [tf.shape(X)[0], self.d, self.d])
    ...
    # for D2U case
    def getBackWeightAndBias(self, iStep, name_of_scope: str = "NetWorkGam"):
        Weights = []
        Bias = []
        with tf.compat.v1.variable_scope(name_of_scope + str(iStep), reuse=tf.compat.v1.AUTO_REUSE):
            ...
            for i in np.arange(len(self.layerSize) - 1):
                scopeName = 'enc_fc' + str(i + 2)
                Weights.append(
                    tf.compat.v1.get_variable(scopeName + "/weights", [self.layerSize[i], self.layerSize[i + 1]]))
