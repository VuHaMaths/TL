...
lamb = np.array([1.5, 1.1, 2., 0.8, 0.5, 1.7, 0.9, 1., 0.9], dtype=np.float32)[0:4]
eta = 0.5
theta = np.array([0.1, 0.2, 0.3, 0.4, 0.25, 0.15, 0.18, 0.08, 0.91], dtype=np.float32)[0:4]
gamma = np.array([0.2, 0.15, 0.25, 0.31, 0.4, 0.35, 0.22, 0.4, 0.15], dtype=np.float32)[0:4]
kappa = np.array([1., 0.8, 1.1, 1.3, 0.95, 0.99, 1.02, 1.06, 1.6], dtype=np.float32)[0:4]
sigma = np.ones(9, dtype=np.float32)[0:4]
d = lamb.shape[0] + 1
nbNeuron = d + 10
