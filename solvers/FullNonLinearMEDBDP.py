...
class PDEFNLSolveSimpleLSExp:
    ...
    def buildBSDEUStep(self, iStep, sess, CWeightU=0, CbiasU=0):
        ...
        normX0 = (dic["XPrev"] - self.xInit - tf.tile(
            tf.expand_dims(self.model.renormTrend(self.TStep * iStep), axis=0), [sample_size, 1])) / rescale
        if (iStep < (self.nbStep - 1)):
            dic["U"], dic["Z"], dic["Gam"] = self.networkUZGam.createNetworkWithInitializer(normX0, iStep, CWeightU,
                                                                                            CbiasU, rescale)
            is_trained_from_zero = True
        else:
        ...
        # Y
        YNext = dic["U"] + self.TStep * (
                    0.5 * tf.einsum('ij,ij->i', sig, tf.linalg.diag_part(dic["GamExt"])) - self.model.fDW(
                iStep * self.TStep, dic["XPrev"], dic["U"], dic["Z"], dic["GamExt"])) + tf.reduce_sum(
            tf.multiply(dic["Z"], dic["dWSig"]), axis=1)
        # Loss
        dic["Loss"] = tf.reduce_mean(tf.pow(dic["UNext"] - YNext, 2))

        # Havu: Freeze layers
        if is_trained_from_zero:
            self.networkUZGam.preload_weights(iStep, sess)
            self.networkUZGam.freeze_layers(iStep)

        dic["train"] = tf.compat.v1.train.AdamOptimizer(learning_rate=dic["LRate"]).minimize(dic["Loss"])
        sess.run(tf.compat.v1.global_variables_initializer())
        # Init weights
        # Havu: load pretrained parameters
        ...
        if is_trained_from_zero:
            self.networkUZGam.preload_weights(iStep, sess)
        # Havu: print command to control trainable parameters
        print('--------------')
    ...
    # BSDE at time 0
    def buildBSDEUStep0(self, Y0_initializer, Z0_initializer, Gam0_initializer, sess):
        is_trained_from_zero = True
        dic = {}
        dic["LRate"] = tf.compat.v1.placeholder(tf.float32, shape=[], name="learning_rate")
        dic["UNext"] = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None], name='UNext')
        ...
        # Y
        YNext = Y + self.TStep * (
                    0.5 * tf.einsum('ij,ij->i', sig, tf.linalg.diag_part(dic["GamExt"])) - self.model.fDW(0., tXInit, Y,
                                                                                                          Z, dic[
                                                                                                              "GamExt"])) + tf.reduce_sum(
            tf.multiply(Z, dic["dWSig"]), axis=1)
        dic["Loss"] = tf.reduce_mean(tf.pow(dic["UNext"] - YNext, 2))
        dic["train"] = tf.compat.v1.train.AdamOptimizer(learning_rate=dic["LRate"]).minimize(dic["Loss"])
    ...
    def plot(self, iStep, sessUPrev, dicoUPrev, batchSizeVal, thePlot):
        ...
        uVal, zVal, gamVal = sessUPrev.run([dicoUPrev["U"], dicoUPrev["Z"], dicoUPrev["Gam"]],
                                           feed_dict={dicoUPrev["XPrev"]: x})
    ...
    def BuildAndtrainML(self, batchSize, batchSizeVal, num_epoch=100, num_epochExtNoLast=100, num_epochExtLast=400,
        ...
        with gCurr.as_default():
            ...
            dicoBSDE = self.buildBSDEUStep(self.nbStep - 1, sess)
            ...
            for iout in range(num_epochExt):
                ...
                for epoch in range(num_epoch):
                    ...
                    wSigT = self.model.getVolByRandom(timeValue, xPrev,
                                                      np.random.normal(size=[batchSize, self.d])) * np.sqrt(self.TStep)
                ...
                wSigT = self.model.getVolByRandom(timeValue, xPrev,
                                                  np.random.normal(size=[batchSizeVal, self.d])) * np.sqrt(self.TStep)
                xNext = self.model.getOneStepFromValuesDw(xPrev, timeValue, self.TStep, wSigT)
        ...
        for iStep in range(self.nbStep - 2, 0, -1):
            ...
            with gCurr.as_default():
                ...
                dicoBSDE = self.buildBSDEUStep(iStep, sess, CWeightU, CbiasU)
                # initialize variables
                # sess.run(tf.compat.v1.global_variables_initializer())
                # saver
                saver = tf.compat.v1.train.Saver()
                ...
                for iout in range(num_epochExt):
                    ...
                    for epoch in range(num_epoch):
                        ...
                        wSigT = self.model.getVolByRandom(timeValue, xPrev,
                                                          np.random.normal(size=[batchSize, self.d])) * np.sqrt(
                            self.TStep)
                        xNext = self.model.getOneStepFromValuesDw(xPrev, timeValue, self.TStep, wSigT)
                        xPrevStor = xNext
                        for i in range(len(dicoList) - 1, -1, -1):
                            tLoc = timeValue + (len(dicoList) - i) * self.TStep
                            wSigTLoc = self.model.getVolByRandom(tLoc, xPrevStor,
                                                                 np.random.normal(size=[batchSize, self.d])) * np.sqrt(
                                self.TStep)
                            # Estimate unext
                            U, Z, Gam = sessList[i].run([dicoList[i]["U"], dicoList[i]["Z"], dicoList[i]["Gam"]],
                                                        feed_dict={dicoList[i]["XPrev"]: xPrevStor})
                            sig2 = self.model.sigScal * self.model.sigScal

                            try:
                                UNext = UNext - self.TStep * (
                                        0.5 * np.einsum('i,jii->j', sig2, Gam) - self.model.fDWNumpy(tLoc,
                                                                                                     xPrevStor, U,
                                                                                                     Z,
                                                                                                     Gam)) - np.sum(
                                    np.multiply(Z, wSigTLoc), axis=1)
                        ...
                        GamExt = sessList[-1].run(dicoList[-1]["Gam"],
                                                  feed_dict={dicoList[-1]["XPrev"]: xPrev})  # previous date
                        # calculate U DU
                        sess.run(dicoBSDE["train"], feed_dict={dicoBSDE["UNext"]: UNext,
                                                               dicoBSDE["GamExt"]: GamExt,
                                                               dicoBSDE["XPrev"]: xPrev,
                    ...
                    wSigT = self.model.getVolByRandom(timeValue, xPrev,
                                                      np.random.normal(size=[batchSizeVal, self.d])) * np.sqrt(
                        self.TStep)
                    xNext = self.model.getOneStepFromValuesDw(xPrev, timeValue, self.TStep, wSigT)
                    xPrevStor = xNext
                    for i in range(len(dicoList) - 1, -1, -1):
                        ...
                        try:
                            UNext = UNext - self.TStep * (
                                    0.5 * np.einsum('i,jii->j', sig2, Gam) - self.model.fDWNumpy(tLoc, xPrevStor, U,
                                                                                                 Z, Gam)) - np.sum(
                                np.multiply(Z, wSigTLoc), axis=1)
                        except ValueError as e:
                            UNext = UNext - self.TStep * (
                                    0.5 * np.einsum('i,jii->j', [sig2], Gam) - self.model.fDWNumpy(tLoc, xPrevStor,
                                                                                                   U, Z,
                                                                                                   Gam)) - np.sum(
                                np.multiply(Z, wSigTLoc), axis=1)
        ...
        with gCurr.as_default():
            ...
            uNext, ZNext, GamNext = sessList[-1].run([dicoList[-1]["U"], dicoList[-1]["Z"], dicoList[-1]["Gam"]],
                                                     feed_dict={dicoList[-1]["XPrev"]: xNext})
            ...
            # build graph
            dicoBSDE = self.buildBSDEUStep0(Y0_initializer, Y0_initializer, Gam0_initializer, sess)
            # initialize variables
            sess.run(tf.compat.v1.global_variables_initializer())
            # saver
            saver = tf.compat.v1.train.Saver()
            ...
            for iout in range(num_epochExt):
                ...
                for epoch in range(num_epoch):
                    UNext = np.zeros(batchSize)
                    wSigT = self.model.getVolByRandom(0., xInit, np.random.normal(size=[batchSize, self.d])) * np.sqrt(
                        self.TStep)
                    ...
                    for i in range(len(dicoList) - 1, -1, -1):
                        ...
                        sig2 = self.model.sigScal * self.model.sigScal
                        try:
                            # Deals with single digit array operation error
                            UNext = UNext - self.TStep * (
                                    0.5 * np.einsum('i,jii->j', sig2, Gam) - self.model.fDWNumpy(tLoc, xPrevStor, U,
                                                                                                 Z, Gam)) - np.sum(
                                np.multiply(Z, wSigTLoc), axis=1)
                    ...
                    UNext = UNext + self.model.g(xPrevStor)
                    GamExt = sessList[-1].run(dicoList[-1]["Gam"],
                                              feed_dict={dicoList[-1]["XPrev"]: xInit})  # previous date
                ...
                UNext = np.zeros(batchSizeVal)
                wSigT = self.model.getVolByRandom(0., xInitVal,
                                                  np.random.normal(size=[batchSizeVal, self.d])) * np.sqrt(self.TStep)
                ...
                for i in range(len(dicoList) - 1, -1, -1):
                    ...
                    try:
                        UNext = UNext - self.TStep * (
                                0.5 * np.einsum('i,jii->j', sig2, Gam) - self.model.fDWNumpy(tLoc, xPrevStor, U, Z,
                                                                                             Gam)) - np.sum(
                            np.multiply(Z, wSigTLoc), axis=1)
                    except ValueError as e:
                        UNext = UNext - self.TStep * (
                ...
                UNext = UNext + self.model.g(xPrevStor)
                GamExt = sessList[-1].run(dicoList[-1]["Gam"],
                                          feed_dict={dicoList[-1]["XPrev"]: xInitVal})  # previous date
