...
class PDEFNLSolveBaseGPU:
    # initial constructor
    def __init__(self, model, T, nbStepUDU, nbStepGam, networkUZ, networkGam, initialLearningRateLast=1e-2,
                 initialLearningRateNoLast=0.005):
    ...
    # Havu: general BSDE resolution, added a 'sess' as input parameter, which we need for using saved parameters
    # added freeze layer and preloading parameters method
    def buildBSDEUStep(self, iStep, ListWeightUZ, ListBiasUZ, ListWeightGam, ListBiasGam, sess):
        ...
        dic["RandG"] = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.d, self.nbStepUDU - iStep],
                                                name='randG')
        # Sample size
        ...
        normX0 = tf.einsum('ij,j->ij', (dic["XPrev"] - self.xInit - mu * self.TStepUDU * iStep),
                           tf.math.reciprocal(tf.constant(rescale, dtype=tf.float32)))
        if (iStep < (self.nbStepUDU - 1)):
            dic["U"], dic["Z"] = self.networkUZ.createNetworkWithInitializer(normX0, iStep, ListWeightUZ[-1],
                                                                             ListBiasUZ[-1], 0.)
            is_trained_from_zero = True
        else:
        ...
        # Y
        YNext = dic["U"] + self.TStepUDU * (0.5 * tf.reduce_sum(
            tf.einsum('j,ij->ij', tf.constant(sig ** 2, dtype=tf.float32), tf.linalg.diag_part(GamExt)),
            axis=1) - self.model.fDW(iStep * self.TStepUDU, dic["XPrev"], dic["U"], dic["Z"], GamExt)) + tf.reduce_sum(
            tf.multiply(dic["Z"],
                        tf.einsum('j,ij->ij', tf.constant(sig, dtype=tf.float32), dic["RandG"][:, :, 0] * sqrtDt)),
            axis=1)

        XNext = dic["XPrev"] + mu * self.TStepUDU + tf.einsum('j,ij->ij', tf.constant(sig, dtype=tf.float32),
                                                              dic["RandG"][:, :, 0] * sqrtDt)
        iStepLoc = iStep + 1
        for i in range(len(ListWeightUZ) - 1, -1, -1):
            iReverse = len(ListWeightUZ) - i
            normX0 = tf.einsum('ij,j->ij', XNext - self.xInit - mu * self.TStepUDU * iStepLoc, tf.math.reciprocal(
                tf.constant(sig * np.sqrt(self.TStepUDU * iStepLoc), dtype=tf.float32)))
            ...
            Gam = self.networkGam.createNetworkNotTrainable(normX0, iStepLoc, ListWeightGam[idecGam],
                                                            ListBiasGam[idecGam])
            YNext = YNext + self.TStepUDU * (0.5 * tf.reduce_sum(
                tf.einsum('j,ij->ij', tf.constant(sig ** 2, dtype=tf.float32), tf.linalg.diag_part(Gam)),
                axis=1) - self.model.fDW(iStepLoc * self.TStepUDU, XNext, U, Z, Gam)) + tf.reduce_sum(tf.multiply(Z,
                     tf.einsum('j,ij->ij', tf.constant(sig, dtype=tf.float32), dic[ "RandG"][ :, :,iReverse] * sqrtDt)),
                                                                                                      axis=1)
            XNext = XNext + mu * self.TStepUDU + tf.einsum('j,ij->ij', tf.constant(sig, dtype=tf.float32),
                                                           dic["RandG"][:, :, iReverse] * sqrtDt)
            iStepLoc = iStepLoc + 1
        # Loss
        ...
        # Havu: load trained parameters
        if is_trained_from_zero:
            self.networkGam.preload_weights(iStep, sess)
            self.networkUZ.preload_weights(iStep, sess)

        # Havu: print command for controlling purposes
        print('--------------')
        print('Trainable parameters: ', tf.get_collection_ref(tf.GraphKeys.TRAINABLE_VARIABLES))
        print('--------------')
        return dic
    ...
    def buildBSDEUStep0(self, ListWeightUZ, ListBiasUZ, ListWeightGam, ListBiasGam, Y0_initializer, Z0_initializer,
        ...
        sqrtDt = np.sqrt(self.TStepUDU)
        # Y
        YNext = Y + self.TStepUDU * (0.5 * tf.reduce_sum(
            tf.einsum('j,ij->ij', tf.constant(sig ** 2, dtype=tf.float32), tf.linalg.diag_part(gam)),
            axis=1) - self.model.fDW(0., tXInit, Y, Z, gam)) + tf.reduce_sum(
            tf.multiply(Z, tf.einsum('j,ij->ij', tf.constant(sig, dtype=tf.float32), dic["RandG"][:, :, 0] * sqrtDt)),
            axis=1)

        XNext = tXInit + mu * self.TStepUDU + tf.einsum('j,ij->ij', tf.constant(sig, dtype=tf.float32),
                                                        dic["RandG"][:, :, 0] * sqrtDt)
        iStepLoc = 1
        for i in range(len(ListWeightUZ) - 1, -1, -1):
            iReverse = len(ListWeightUZ) - i
            ...
            YNext = YNext + self.TStepUDU * (0.5 * tf.reduce_sum(
                tf.einsum('j,ij->ij', tf.constant(sig ** 2, dtype=tf.float32), tf.linalg.diag_part(Gam)),
                axis=1) - self.model.fDW(iStepLoc * self.TStepUDU, XNext, U, Z, Gam)) + tf.reduce_sum(
                tf.multiply(Z, sig * dic["RandG"][:, :, iReverse] * sqrtDt), axis=1)
            XNext = XNext + mu * self.TStepUDU + tf.einsum('j,ij->ij', tf.constant(sig, dtype=tf.float32),
                                                           dic["RandG"][:, :, iReverse] * sqrtDt)
            iStepLoc = iStepLoc + 1
        # Loss
    ...
    def plot(self, iStep, sessUPrev, dicoUPrev, batchSizeVal, thePlot, gamVal):
        ...
        if self.model.name == "merton":
            for i in range(self.model.lamb.shape[0]):
                plt.plot(x[:, 0],
                         -self.model.lamb[i] * self.model.theta[i] / np.exp(self.model.theta[i]) * zVal[:, 0] / gamVal[
                                                                                                                :, 0,
                                                                                                                0], '.',
                         label="Control   ", markersize=1)
                plt.plot(x[:, 0], self.model.lamb[i] * self.model.theta[i] / np.exp(
                    self.model.theta[i]) / self.model.eta * np.ones(x.shape[0]), '.', label="Control Analytic",
                         markersize=1)
                P.xlabel(r"$x$")
                ...
                P.figure()
                np.savetxt(thePlot + 'control' + str(iStep),
                           -self.model.lamb[i] * self.model.theta[i] / np.exp(self.model.theta[i]) * zVal[:,
                                                                                                     0] / gamVal[:, 0,
                                                                                                          0])
                np.savetxt(thePlot + 'truecontrol' + str(iStep), self.model.lamb[i] * self.model.theta[i] / np.exp(
                    self.model.theta[i]) / self.model.eta * np.ones(x.shape[0]))
    ...
    # buikd and train
    def BuildAndtrainML(self, batchSize, batchSizeVal, num_epoch=100, num_epochExtNoLast=100, num_epochExtLast=400,
                        min_decrease_rate=0.05, nbOuterLearning=20, thePlot="", saveDir="./", baseFile=""):
        ...
        for iGam in range(self.nbStepGam, 0, -1):
            ...
            with gCurrGam.as_default():
                ...
                dicoGam = self.buildGamStep(iGam, ListWeightUZ, ListBiasUZ, ListWeightGam, ListBiasGam, sessGam)
                print("CREATED GAM ", iGam)
                # initialize variables, Havu: excluded, since sess.run will reset preloaded parameters
                ...
                if (thePlot != "" and (
                        iGam * self.nbStepGamStab == 1 or iGam * self.nbStepGamStab == 60 or iGam * self.nbStepGamStab == 4)):
                    if (self.d == 1):
                    ...
                    with gCurr.as_default():
                        ...
                        for iout in range(num_epochExt):
                            ...
                            for epoch in range(num_epoch):
                                ...
                                # calculate U DU
                                sessBSDE.run(dicoBSDE["train"],
                                             feed_dict={dicoBSDE["RandG"]: NG, dicoBSDE["XPrev"]: xPrev,
                            ...
                            # calculate U DU
                            valLoss = sessBSDE.run(dicoBSDE["Loss"],
                                                   feed_dict={dicoBSDE["XPrev"]: xPrev, dicoBSDE["RandG"]: NG})
                        ...
                        # get back weight
                        CWeightU, CbiasU = self.networkUZ.getWeights(sessBSDE, dicoBSDE["weightLoc"],
                                                                     dicoBSDE["biasLoc"])
                        # store session
                        ...
                        if ((thePlot != "") and ((iStep == self.nbStepUDU / 2) or (iStep == 1))):  # -1 in Xavier's code
                            if (self.d == 1):
                                # Set gamval here for edge case activation
                                gamval = self.plotGam(iGam * self.nbStepGamStab, sessGam, dicoGam, batchSizeVal,
                                                      thePlot)
        ...
        with gCurr.as_default():
            ...
            # for Gam
            # build graph
            dicoBSDE = self.buildBSDEUStep0(ListWeightUZ, ListBiasUZ, ListWeightGam, ListBiasGam, Y0_initializer,
                                            Z0_initializer, sessBSDE)
            ...
            for iout in range(num_epochExt):
                ...
                for epoch in range(num_epoch):
                    ...
                    sessBSDE.run(dicoBSDE["train"], feed_dict={dicoBSDE["Gam0"]: Gam0, dicoBSDE["RandG"]: NG,
                                                               dicoBSDE["LRate"]: theLearningRate})
